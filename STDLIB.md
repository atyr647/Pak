...
The authoritative source for the module surface is the `MODULE_API` dict in
`pak/codegen.py`. This document is generated to match it: **37 modules,
~304 functions**. The checker and typechecker derive their module set from the
same `MODULE_API`, so if a function is not listed here it is not lowered to C
(it will silently pass `pak check` as an unknown method but emit invalid C).
...
| `rdpq.triangle` | `(...)` | Draw a triangle (`rdpq_triangle`, see libdragon) |
...
| `rdpq.set_tile` | `(...)` | Configure a tile descriptor (see libdragon) |
...
If a module or function is not in this document (and not in `MODULE_API`), it
