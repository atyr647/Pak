...
printf 'SECTIONS{ . = 0x80000400; .text:{*(.text)} .rodata:{*(.rodata)} }\n' > "$T/l.ld"
"$LD" -T "$T/l.ld" -o "$T/t.elf" "$T/t.o"; "$OC" -O binary -j .text -j .rodata "$T/t.elf" "$T/t.bin"
B=$(od -An -tx1 "$T/t.bin"|tr -d ' \n')
M=$(tclsh "$HERE/n64asm_dump.tcl" "$T/t.s")
[ "$B" = "$M" ] && echo "n64asm parity: MATCH (byte-exact vs binutils)" || { echo "n64asm parity: MISMATCH"; echo "B:$B"; echo "M:$M"; exit 1; }
